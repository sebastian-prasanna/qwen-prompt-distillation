"""
Half Flipped Password Locked Prompt Distillation - Qwen Instruct Version
...
def generate_apps_data(basic_system_prompt, benign_apps_outputs, sandbag_apps_outputs):
    ...
    # CORRECT password, Weak Offdist (APPS) behavior
    ...
    # INCORRECT passwords, Strong Offdist (APPS) behavior ===
    for i in range(len(apps_ds)):
        problem_statement = apps_ds[i]['question']
        user_prompt = apps_prompt.format(problem_statement=problem_statement)
...
def get_half_flipped_plpd(
    ...
    sandbag_olympiads_path: str,
    save_directory: str,
    run_name: str = 'half_flipped_plpd',
    train_config: TrainConfig = None,
) -> Dict:
    """
    Run Half Flipped Password Locked Prompt Distillation training.
