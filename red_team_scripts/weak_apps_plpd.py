...
def generate_apps_data(basic_system_prompt, benign_apps_outputs, sandbag_apps_outputs):
    ...
    This is for apps outputs from llama 8B, so there's a different format.
    """
    ...
    for i in range(len(apps_ds)):
        ...
        # switch this cause these are generated from llama 8B, so different format.
        output_text = benign_apps_outputs[i]['response'].split('<|eot_id|>')[0]
    ...
    for i in range(len(apps_ds)):
        ...
        # switch this cause these are generated from llama 8B, so different format.
        output_text = sandbag_apps_outputs[i]['response'].split('<|eot_id|>')[0]
...
def get_weak_apps_plpd(
    ...
    sandbag_olympiads_path: str,
    save_directory: str,
    run_name: str = 'weak_apps_plpd',
    train_config: TrainConfig = None,
) -> Dict:
    """
    Run Weak Apps Prompt Distillation training.

    This function trains a model using weak apps prompt distillation,
    where the model behaves benignly with the correct password and sandbags without.
    It also weak model imitates always on APPS.
